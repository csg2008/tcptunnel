...
// NewTCPProxy 创建TCP代理
func NewTCPProxy(listenAddress string, targetAddress string, proxyAddress string, dialTimeout time.Duration, keepAlivePeriod time.Duration, sigChan chan os.Signal) *TCPProxy {
	return &TCPProxy{
...
func (c *TCPProxy) connCopy(dst, src net.Conn, copyDone chan struct{}) {
	defer c.wg.Done()
	defer func() {
		copyDone <- struct{}{}
	}()
	...
	if err != nil {
		...
		log.Printf("failed to copy connection from %s to %s: %s", src.RemoteAddr(), dst.RemoteAddr(), err)
	}
}

// Run 启动代理
func (c *TCPProxy) Run() error {
	...
	log.Printf("Listening port opened on %s", c.listenAddress)
	...
	case <-c.signal:
		log.Printf("received shutdown signal from user")
	...
	log.Printf("stopping proxy TCPProxy: %s", c.listenAddress)
	...
	case <-time.After(time.Duration(10) * time.Second):
		log.Printf("some goroutines will be stopped forcefully")
	}
	return <-c.errChan

}

func (c *TCPProxy) serve(listener net.Listener, dialer proxy.Dialer) error {
	// accept loop
	for {
		...
		log.Printf("accepted connection from %s on %s", accepted.RemoteAddr(), accepted.LocalAddr())

		// when accepted, dial remote
		dialed, err := dialer.Dial("tcp", c.targetAddress)
		if err != nil {
			log.Printf("error dialing remote target: %s", err)
			accepted.Close()
			continue
		}
...
func (c *TCPProxy) handleConn(accepted net.Conn, remote net.Conn) {
	defer c.wg.Done()
	defer accepted.Close()
	defer remote.Close()

	log.Printf("tunneling connection from %s to %s", accepted.RemoteAddr(), remote.RemoteAddr())
...
func (c *TCPProxy) duplexCopy(conn, rConn net.Conn, ch chan struct{}) {
	defer c.wg.Done()
